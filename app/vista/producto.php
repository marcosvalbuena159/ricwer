<?php
    ...
    if($id == '' || $token == ''){
        ...
        if ($token == $token_tmp){
            $sql = $conn->prepare("SELECT count(idProductos) FROM `producto` WHERE id=? AND activo=1");
            $sql->execute([$id]);
            if ($sql->fetchColumn() > 0){
                $sql = $conn->prepare("SELECT `nombreProducto`, `descripcionProducto`, `precioProducto`, `descuentoProducto` FROM `producto` WHERE id=? AND activo=1 LIMIT 1");
                $sql->execute([$id]);
                $row = $sql->fetch(PDO::FETCH_ASSOC);
                $nombre = $row['nombreProducto'];
                $descripcion = $row['descipcionProducto'];
                $precio = $row['precioProducto'];
                $descuento = $row['descuentoProducto'];
                $precio_desc = $precio - (($precio * $descuento) / 100);
                $dir_imagenes = '../../public/img/00'.$id.'.jpeg';
                $rutaimg = $dir_imagenes . '../../public/img/001.jpeg';
                if(!file_exists($rutaimg)){
                    $rutaimg= '../../public/img/001.jpeg';
                }
                $imagenes = array();
                ...
                while(($archivo = $dir->read()) != false){
                    if($archivo != '../../public/img/001.jpeg' && (strpos($archivo, 'jepg') || strpos($archivo, 'jpg'))){
...
<main>
            <div class="container">
                         ...
                         <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="1" aria-label="Slide 2"></button>
                          ...
                          <div class="carousel-inner">
                            <div class="carousel-item active">
                              <img src="../../public/img/001.jpeg" class="d-block w-100" alt="...">
                ...
                <?php if($descuento > 0) {?>
                    <span><del><?php echo MONEDA . number_format($precio, 2, '.', ',');?></del></span>
                    <span><?php echo MONEDA . number_format($precio_desc, 2, '.', ',');?></span>
                    <small class="text-sucess"><?php echo $descuento; ?></small>
                <?php } else {?>
				<span><?php echo MONEDA . number_format($precio, 2, '.', ',');?>% de descuento</span>
                <?php } ?>
        ...
        <div class="contenedor-items">
            ...
            <div class="item">
                ...
                <img src="../../public/img/005.jpeg" alt="" class="img-item">
