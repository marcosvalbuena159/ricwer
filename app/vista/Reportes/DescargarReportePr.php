...
$pdf->Cell(180,6,'LISTA DE PRODUCTOS',0,0,'C');
...
$pdf->Cell(44,6,'PRECIO',1,0,'C',1);
$pdf->Cell(28,6,'CATEGORIA',1,1,'C',1); 
...
$sqlTrabajado = ("SELECT * FROM `producto`");
//$sqlTrabajadores = ("SELECT * FROM trabajadores");
$query = mysqli_query($conn, $sqlTrabajado);

while ($dataRow = mysqli_fetch_array($query)) {
    $pdf->Cell(30,6,($dataRow['idProducto']),1,0,'C');
    $pdf->Cell(35,6,$dataRow['descripcionProducto'],1,0,'C');
    $pdf->Cell(44,6,($dataRow['precioProducto']),1,0,'C');
    $pdf->Cell(28,6,$dataRow['categoriaProducto'],1,1,'C');
}
...
$pdf->Output('Resumen_Productos_'.date('d_m_y').'.pdf', 'I'); 
